import { pgTable, foreignKey, serial, text, boolean, integer, uuid, timestamp, real, bigint, unique, date, doublePrecision, numeric, primaryKey, pgSequence, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const categoryEnum637Cb4B7 = pgEnum("category_enum_637cb4b7", ['On-ramp', 'Off-ramp'])
export const categoryEnumAd2Fad7F = pgEnum("category_enum_ad2fad7f", ['On-ramp', 'Off-ramp'])
...
export const recievingMethodEnum17B6B816 = pgEnum("recieving_method_enum_17b6b816", ['Telegram', 'WhatsApp', 'Order Form'])
export const recipientCurrencyEnum9767781C = pgEnum("recipient_currency_enum_9767781c", ['USDT (TRX)', 'USD (FIAT)', 'USDT (ETH)', 'USDC (ETH)', 'MXN (FIAT)'])
export const recipientCurrencyEnumFd3Db64B = pgEnum("recipient_currency_enum_fd3db64b", ['USDT (TRX)', 'USD (FIAT)', 'USDT (ETH)', 'USDC (ETH)', 'MXN (FIAT)'])
export const recipientCurrencyEnumFf577Fa3 = pgEnum("recipient_currency_enum_ff577fa3", ['USDT (TRX)', 'USD (FIAT)', 'USDT (ETH)', 'USDC (ETH)', 'MXN (FIAT)'])
export const senderCurrencyEnum06F7D958 = pgEnum("sender_currency_enum_06f7d958", ['USD (FIAT)', 'USDC (ETH)', 'USDT (ETH)', 'USDT (TRX)', 'MXN (FIAT)'])
export const senderCurrencyEnum1E5C83F7 = pgEnum("sender_currency_enum_1e5c83f7", ['USD (FIAT)', 'USDC (ETH)', 'USDT (ETH)', 'USDT (TRX)', 'MXN (FIAT)'])
export const senderCurrencyEnum5B17Ffbd = pgEnum("sender_currency_enum_5b17ffbd", ['USD (FIAT)', 'USDC (ETH)', 'USDT (ETH)', 'USDT (TRX)', 'MXN (FIAT)'])
export const sendingCurrencyTypeEnumF9F3Ddac = pgEnum("sending_currency_type_enum_f9f3ddac", ['stable', 'fiat'])
export const sourceEnum53B14601 = pgEnum("source_enum_53b14601", ['Manual Sheet'])
export const statusEnum0D232213 = pgEnum("status_enum_0d232213", ['Submitted', 'Raised', 'Sent', 'Received', 'Cancelled', 'Pending'])
export const statusEnum607Af79C = pgEnum("status_enum_607af79c", ['active', 'inactive', 'deleted'])
export const statusEnum7892B369 = pgEnum("status_enum_7892b369", ['COMPLETED', 'AWAITING_COMPLIANCE_REVIEW', 'CANCELLED', 'PAYMENT_PROCESSED', 'PROCESSING_PAYMENT', 'AWAITING_FUNDS', 'IN_COMPLIANCE_REVIEW', 'CREATED'])
export const statusEnumA1Daba09 = pgEnum("status_enum_a1daba09", ['AWAITING_COMPLIANCE_REVIEW', 'PAYMENT_PROCESSED', 'CANCELLED', 'COMPLETED'])
export const statusEnumDc69B788 = pgEnum("status_enum_dc69b788", ['active', 'updated', 'deleted'])
export const statusEnumDef65B9D = pgEnum("status_enum_def65b9d", ['AWAITING_COMPLIANCE_REVIEW', 'PAYMENT_PROCESSED', 'CANCELLED', 'COMPLETED'])
...
export const customerAccounts = pgTable("Customer_Accounts", {
	...
	routingNumberOfIntermediaryBank: text("routing_number_of_intermediary_bank"),
...
export const sumsubIndividuals = pgTable("sumsub_individuals", {
	...
	lastReviewDate: timestamp("last_review_date", { withTimezone: true, mode: 'string' }),
...
export const conduitTransactionsCopy = pgTable("conduit_transactions_copy", {
	id: text().notNull(),
	sender: text(),
	effective: date(),
	...
	return {
		conduitTransactionsCopyIdKey: unique("conduit_transactions_copy_id_key").on(table.id),
	}
});

export const conduitTransactionsCopy2 = pgTable("conduit_transactions_copy2", {
	created: timestamp({ withTimezone: true, mode: 'string' }),
	id: text().notNull(),
	sender: text(),
	type: text(),
	...
	return {
		conduitTransactionsCopy2IdKey: unique("conduit_transactions_copy2_id_key").on(table.id),
	}
});
...
export const currencies = pgTable("currencies", {
	...
	return {
		currenciesCurrencyAbrevKey: unique("currencies_currency_abrev_key").on(table.currencyAbrev),
	}
});
...
export const orderData = pgTable("Order_Data", {
	id: serial().primaryKey().notNull(),
	dateOfRequest: timestamp("date_of_request", { withTimezone: true, mode: 'string' }).default('2024-10-08 16:26:19.406701+00'),
	...
	txnId: text("txn_id"),
	internalResponsiblePerson: internalResponsiblePersonEnumB31B90Bc("internal_responsible_person"),
	status: statusEnum0D232213(),
	datePaymentSent: timestamp("date_payment_sent", { withTimezone: true, mode: 'string' }),
	notes: text(),
...
export const currencyPairs = pgTable("currency_pairs", {
	...
	receivingCurrencyType: receivingCurrencyTypeEnum9F498E43("receiving_currency_type"),
...
export const currencyTradingPairs = pgTable("currency_trading_pairs", {
	receivingCurrency: text("receiving_currency"),
	feeTotal: numeric("fee_total", { precision: 15, scale:  4 }).default('0'),
	category: categoryEnumAd2Fad7F(),
	id: integer().primaryKey().generatedByDefaultAsIdentity({ name: "currency_pairs_copy_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	sendingCurrency: text("sending_currency"),
}, (table) => {
	return {
...
export const volumeSubscriptionPayments = pgTable("volume_subscription_payments", {
	day: date().primaryKey().notNull(),
	arbitrum: doublePrecision(),
	bitcoin: doublePrecision(),
	bsc: doublePrecision(),
...
export const currencyPairs2 = pgTable("currency_pairs_2", {
	...
	return {
		...
		fiatCurrencyCryptoTokenPairsPkey: primaryKey({ columns: [table.fiatCurrencyId, table.cryptoTokenId, table.operationType], name: "fiat_currency_crypto_token_pairs_pkey"}),
	}
});

export const volumeBridgeTransfers = pgTable("volume_bridge_transfers", {
	date: date().notNull(),
	...
	return {
		volumeBridgeTransfersPkey: primaryKey({ columns: [table.date, table.bridgeTransferStatus], name: "volume_bridge_transfers_pkey"}),
	}
});
